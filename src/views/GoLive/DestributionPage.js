...
const DestributionPage = () => {
    ...
    fetch('/.netlify/functions/airtable-getprofile', {
      ...
      .then(function (response) {
        if (response.records[0].fields) {
          ...
          setUserProfile(
            response.records[0].fields.restrictedChannelTags.split(',')
          )
        }
      })
      .catch(error => {
  ...
  const onChannelChanged = (e, key) => {
    ...
    if (
      !ChannelInfo.rtmpLink ||
      !ChannelInfo.streamKey ||
      !ChannelInfo.liveStreamId
    ) {
      toast.error(
        "We can't Go Live for this channel as stream key is not provided. Please update key and try again."
      )
      setdirectLink(false)
      setCreate(false)
    } else {
      ...
      setbutton(true)
      toast.success(
        'Please click Create Direct Link below to stream to selected channel'
      )
    }
  }
            ...
            <div className="table_inner">
              ...
              {create && (
                <ul>
                  <header>
                    <h3>
                      Please link the keys below to OBS and then click Create
                      Direct Link in order to go live on selected channel
                    </h3>
                  </header>
                  <li>Stream Key: {channelList['streamKey']}</li>
                  <li>
                    RTMP Link:{' '}
                    <a
