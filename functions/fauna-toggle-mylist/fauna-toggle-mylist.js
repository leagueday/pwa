  ...
  try {
    await client.query(
      q.Let(
        ...
        q.Update(q.Var('ref'), { data: { my: q.Var('myNext') } })
      )
    )
      ...
      q.Let(
        {
          ...
          myNext: q.Filter(
            ...
            q.Lambda(
              'i',
              q.Or(
                q.Not(q.Equals(q.Select(['id'], q.Var('i')), id)),
                q.Not(q.Equals(q.Select(['kind'], q.Var('i')), kind))
              )
            )
          ),
        },
...
const handler = async (event, context) => {
  ...
  const [status, diagnostic] = await querySubroutine(
    client,
    userEmail,
    kind,
    id
